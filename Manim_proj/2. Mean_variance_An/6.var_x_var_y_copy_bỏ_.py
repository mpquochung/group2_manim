...
class Var_x_var_y_copy(Scene):
    def construct(self):
        ...
        formula_var_text=MathTex("\sigma ^2 _{X+Y}= \sigma^2 _X+\sigma^2_Y")
        formula_var_text.shift(DOWN*1,RIGHT*2)
        
        formula_var=MathTex("Var(X+Y)=Var(X)+Var(Y)")
        formula_var.scale(0.8)
        formula_var.shift(DOWN*2,RIGHT*3)
        formula_var_text_1=Text("(Assuming X and Y are independent!)")
        formula_var_text_1.shift(DOWN*3,RIGHT*3)
        formula_var_text_1.scale(0.5)
        ...
        self.add(group3,group2,chart_xy,group_arrow_xy,formula[6],formula[7],formula[8])
        self.play(Create(formula_var),Create(formula_var_text),Create(formula_var_text_1))
