from manim import *
class Bar_chart_x17(MovingCameraScene):
    def construct(self):
        prob = {    1:0.4,
                    2:0.25,
                    3:0.15,
        ...
        for n in range(1, 8):  # Tạo 8 biểu đồ với n từ 1 đến 8 (X1,X1+X2,...,X1+X2+...+X7)
            ...
            std_left, std_right = self.calculate_std_interval(values)
            chart = self.create_bar_chart(values)
            mean_line_x=self.mean_line(chart, mean_value)
            mean_line_std_left_x=self.mean_line(chart, std_left)
            mean_line_std_right_x=self.mean_line(chart, std_right)
            arrow_left, arrow_right = self.get_mean_std_arrows(chart, mean_value, std_left, std_right)
            group_chart = VGroup(chart, mean_line_x, mean_line_std_left_x, mean_line_std_right_x, arrow_left, arrow_right)
    ...
    def create_bar_chart(self, probability_data):
        ...
        chart = BarChart(
                ...
                bar_names=self.keys,
                bar_colors=[
          interpolate_color(BLUE, GREEN, x/len(self.keys)) 
    ...
    def get_mean_std_arrows(self, chart, mean, std_left, std_right, arrow_color=WHITE):
        ...
        arrow_left = Arrow(start=start, end=end_left, color=arrow_color, buff=0)
        arrow_right = Arrow(start=start, end=end_right, color=arrow_color, buff=0)

        arrow_left.set_stroke(width=2).shift(UP*1)
        arrow_right.set_stroke(width=2).shift(UP*1)

        arrow_left.set_color(RED)
        arrow_right.set_color(RED)
        sigma_text_left = r"\sigma" if self.n == 1 else f"\sqrt {self.n} \sigma"
        sigma_text_right = r"\sigma" if self.n == 1 else f"\sqrt {self.n} \sigma"

        sigma_left = MathTex(sigma_text_left).next_to(arrow_left, UP).set_color(RED)
        sigma_right = MathTex(sigma_text_right).next_to(arrow_right, UP).set_color(RED)
        return VGroup(arrow_left,sigma_left), VGroup(arrow_right,sigma_right)
